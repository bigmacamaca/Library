$(document).ready(function() {
    ...
    $.ajax({
      method: 'GET',
      url : base_url + '/books/api/get_user_books/'+id+ '/', 
      // url: 'http://127.0.0.1:8000/api/books/get_bookDetails/<int:book_id>/',
      beforeSend: function() {
          console.log('before get user books');
      },
      success: function(userBooksData) {
          displayUserBooks(userBooksData);
            ...
            "<div class='card'>" +
              ...
              "<div class='p-4 text-black' style='background-color: #f8f9fa;'>" +
                "<div class='d-flex justify-content-end text-center py-1'>" +
                  "<div>" +
                    ...
                    "<p class='small text-muted mb-0'>Books Owned</p>" +
                  "</div>" +
                "</div>" +
              "</div>" +
              "<div class='card-body p-4 text-black'>" +
                ...
                "<div class='d-flex justify-content-between align-items-center mb-4'>" +
                  "<p class='lead fw-normal mb-0'>Owned Books</p>" +
                "</div>" +
                "<div class='col'>" +
                      "<div id='userBooksList'>" +
                      "</div>" +
                  "</div>" +
                "</div>" +
              "</div>" +
...
function displayUserBooks(userBooksData) {
  var base_url = window.location.origin
  let template = "";
  $.each(userBooksData, function(index, value) {
      console.log(value)
      template += "<br>" +
  ...
  $('#userBooksList').prepend(template)
  console.log(template)
  // document.getElementById('dataDisplay').innerHTML = template
}
